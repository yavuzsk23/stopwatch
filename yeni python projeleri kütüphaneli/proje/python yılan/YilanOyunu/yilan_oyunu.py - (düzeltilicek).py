# YILAN OYUNU - FINAL VERSION 4.0
# Denizli İş Adamları MTAL - Skor Tablosu + 60 FPS
...
# Görevler sistemi
ACHIEVEMENTS = [
    {"id": "food_10", "name": "İlk 10 Yem", "desc": "10 normal yem ye", "target": 10, "icon": "🍎"},
    {"id": "food_50", "name": "Yem Avcısı", "desc": "50 normal yem ye", "target": 50, "icon": "🍏"},
    {"id": "no_wall", "name": "Duvarsız", "desc": "Hiç duvara çarpma", "target": 1, "icon": "🛡️"},
    {"id": "speed_15", "name": "Hız Canavarı", "desc": "Hız 15'e ulaş", "target": 15, "icon": "⚡"},
    {"id": "golden_5", "name": "Altın Avcı", "desc": "5 altın yem ye", "target": 5, "icon": "💰"},
    {"id": "survive_60", "name": "Hayatta Kalma", "desc": "60 saniye hayatta kal", "target": 60, "icon": "⏱️"},
]
...
def main():
    ...
    while True:
        ...
        wall_hits = 0  # Duvara çarpma sayısı
        ...
        while running:
            ...
            # Görevler kontrol
            achievements_progress["food_10"] = normal_foods_eaten
            achievements_progress["food_50"] = normal_foods_eaten
            achievements_progress["no_wall"] = 1 if wall_hits == 0 else 0
            achievements_progress["speed_15"] = 1 if speed >= 15 else 0
            achievements_progress["golden_5"] = golden_foods_eaten
            achievements_progress["survive_60"] = int(time_played)
            
            for ach in ACHIEVEMENTS:
                if ach["id"] not in achievements_completed:
                    if achievements_progress[ach["id"]] >= ach["target"]:
                        achievements_completed.append(ach["id"])
                        # Başarım popup
                        for _ in range(30):
                            particles.append(Particle(WIDTH//2, 150, GOLD))
            ...
            # Engeller (15. puan) - YıLANIN İÇİNE SPAWN OLMASIN
            if score >= 15 and len(static_obstacles) == 0:
                attempts = 0
                while len(static_obstacles) < 8 and attempts < 100:
                    obs_x = random.randint(5, (WIDTH - CELL) // CELL - 5) * CELL
                    obs_y = random.randint((UI_HEIGHT // CELL) + 5, (HEIGHT // CELL) - 5) * CELL
                    obs = (obs_x, obs_y)
                    
                    # Yılanın yakınında olmasın
                    too_close = False
                    for seg in snake:
                        dist = math.sqrt((obs[0] - seg[0])**2 + (obs[1] - seg[1])**2)
                        if dist < CELL * 5:  # En az 5 hücre uzakta
                            too_close = True
                            break
                    
                    if not too_close and obs != food and obs not in snake:
            ...
            # 5 sn kontrol - ALTYN YEM BUG DÜZELTİLDİ
            if golden_food and golden_spawn_time and (current_time - golden_spawn_time) > 5:
                golden_food = None
                golden_spawn_time = None
            if ghost_powerup and ghost_spawn_time and (current_time - ghost_spawn_time) > 5:
                ghost_powerup = None
                ghost_spawn_time = None
            if magnet_powerup and magnet_spawn_time and (current_time - magnet_spawn_time) > 5:
                magnet_powerup = None
                magnet_spawn_time = None
            if tail_cut_powerup and tail_cut_spawn_time and (current_time - tail_cut_spawn_time) > 5:
                tail_cut_powerup = None
                tail_cut_spawn_time = None
            if double_powerup and double_spawn_time and (current_time - double_spawn_time) > 5:
                double_powerup = None
                double_spawn_time = None
            if wall_powerup and wall_spawn_time and (current_time - wall_spawn_time) > 5:
                wall_powerup = None
                wall_spawn_time = None
            if shield_powerup and shield_spawn_time and (current_time - shield_spawn_time) > 5:
                shield_powerup = None
                shield_spawn_time = None
            if stealth_powerup and stealth_spawn_time and (current_time - stealth_spawn_time) > 5:
                stealth_powerup = None
                stealth_spawn_time = None
            ...
            if move_accumulator >= move_interval:
                ...
                if not ghost_active:
                    ...
                    for seg in snake[1:]:  # İlk segment hariç
                        seg_rect = pygame.Rect(seg[0], seg[1], CELL, CELL)
                ...
                # Engel - KESİN DÜZELTME
                if not ghost_active and not shield_active:
                    head_rect = pygame.Rect(new_head[0], new_head[1], CELL, CELL)
                    for obs in static_obstacles:
                        obs_rect = pygame.Rect(obs[0], obs[1], CELL, CELL)
                ...
                if not ate and golden_food:
                    ...
                    if head_rect.colliderect(golden_rect):
                        ...
                        golden_food = None
                        golden_spawn_time = None  # BURAYI EKLEDIM
                        screen_shake()
            ...
            for fb in fireballs[:]:
                ...
                if not ghost_active:
                    ...
                    for i, seg in enumerate(snake):
                        seg_rect = pygame.Rect(seg[0], seg[1], CELL, CELL)
                        fb_rect = pygame.Rect(int(fb.x) - 6, int(fb.y) - 6, 12, 12)
                        if seg_rect.colliderect(fb_rect):
                            hit_segment = i
                            break
                    
                    if hit_segment is not None:
                        # Animasyonlu silme - vurduğu yerden sonrasını sil
                        removed_segments = snake[hit_segment:]
                        snake = snake[:hit_segment] if hit_segment > 0 else [snake[0]]
                        
                        # Parçacık efekti
                        for seg in removed_segments:
                            for _ in range(5):
                                particles.append(Particle(seg[0] + CELL//2, seg[1] + CELL//2, ORANGE))
            ...
            # Yemler - ZıPLAYAN ELM A
            pygame.draw.circle(screen, RED, (food[0] + CELL//2, food[1] + CELL//2 - int(food_bounce)), CELL//2)
            pygame.draw.circle(screen, (255, 100, 100), (food[0] + CELL//2, food[1] + CELL//2 - int(food_bounce)), CELL//2, 2)
            pygame.draw.rect(screen, BROWN, (food[0] + CELL//2 - 1, food[1] - int(food_bounce), 2, 5))
            pygame.draw.ellipse(screen, GREEN, (food[0] + CELL//2 + 2, food[1] - 2 - int(food_bounce), 6, 4))
            
            if golden_food:
                remaining = max(0, 5 - (current_time - golden_spawn_time)) if golden_spawn_time else 5
                draw_powerup_icon(golden_food[0], golden_food[1], "golden", GOLD, pulse, remaining)
            
            if ghost_powerup:
                remaining = max(0, 5 - (current_time - ghost_spawn_time)) if ghost_spawn_time else 5
                draw_powerup_icon(ghost_powerup[0], ghost_powerup[1], "ghost", TURQUOISE, pulse, remaining)
            
            if magnet_powerup:
                remaining = max(0, 5 - (current_time - magnet_spawn_time)) if magnet_spawn_time else 5
                draw_powerup_icon(magnet_powerup[0], magnet_powerup[1], "magnet", PINK, pulse, remaining)
            
            if tail_cut_powerup:
                remaining = max(0, 5 - (current_time - tail_cut_spawn_time)) if tail_cut_spawn_time else 5
                draw_powerup_icon(tail_cut_powerup[0], tail_cut_powerup[1], "tail_cut", WHITE, pulse, remaining)
            
            if double_powerup:
                remaining = max(0, 5 - (current_time - double_spawn_time)) if double_spawn_time else 5
                draw_powerup_icon(double_powerup[0], double_powerup[1], "double", ORANGE, pulse, remaining)
            
            if wall_powerup:
                remaining = max(0, 5 - (current_time - wall_spawn_time)) if wall_spawn_time else 5
                draw_powerup_icon(wall_powerup[0], wall_powerup[1], "wall", CYAN, pulse, remaining)
            
            if shield_powerup:
                remaining = max(0, 5 - (current_time - shield_spawn_time)) if shield_spawn_time else 5
                draw_powerup_icon(shield_powerup[0], shield_powerup[1], "shield", BLUE, pulse, remaining)
            
            if stealth_powerup:
                remaining = max(0, 5 - (current_time - stealth_spawn_time)) if stealth_spawn_time else 5
            ...
            # Başarımlar - Sağ alt köşe
            ach_y = HEIGHT - 120
            completed_count = len(achievements_completed)
            draw_text(f"BASARIMLAR: {completed_count}/{len(ACHIEVEMENTS)}", (WIDTH - 200, ach_y), GOLD, FONT_TINY)
            ach_y += 15
            for ach in ACHIEVEMENTS[:3]:  # İlk 3 başarım
                if ach["id"] in achievements_completed:
                    draw_text(f"✓ {ach['name']}", (WIDTH - 200, ach_y), GREEN, FONT_TINY)
                else:
                    progress = achievements_progress[ach["id"]]
                    target = ach["target"]
                    draw_text(f"  {ach['name']} ({progress}/{target})", (WIDTH - 200, ach_y), GRAY, FONT_TINY)
                ach_y += 12
